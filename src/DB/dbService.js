...
export const findById = async({model, id = "", select="", populate = []}={}) =>{
    ...
    const document = await model.findById(new mongoose.Types.ObjectId(id)).select(select).populate(populate)
    return document
}
...
export const count = async({model, filter = {}, select="", populate = []}={}) =>{

    const document = await model.count(filter).select(select).populate(populate).lean()
    return document
}

export const exists = async({model, filter = {}, select="", populate = []}={}) =>{

    const document = await model.exists(filter).select(select).populate(populate).lean()
    return document
}